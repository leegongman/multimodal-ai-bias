...
PORT="${VLLM_PORT:-8000}"
LOG="$ROOT/logs/qwen35-9b-vllm-hires-server.log"
PID_FILE="$ROOT/logs/qwen35-9b-vllm-hires-server.pid"
...
nohup env PATH="$VENV/bin:$PATH" \
  ...
  --mm-processor-kwargs '{"max_pixels":451584,"min_pixels":200704}' \
  >"$LOG" 2>&1 < /dev/null &
pid=$!
printf '%s\n' "$pid" > "$PID_FILE"
