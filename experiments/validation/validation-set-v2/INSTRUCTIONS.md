...
## 2. 파일 설명 (이 폴더 = `data/local-validation/v2/`)
...
ls data/local-validation/v2/valset.csv data/local-validation/v2/answer_key.csv
...
```bash
python3 data/local-validation/v2/codex_handoff/run_valset.py \
  --valset-dir data/local-validation/v2 \
  --image-dir  data/shadow-private/image-pool-v1/images \
  --base-url   http://127.0.0.1:8000/v1 \
  --model-name Qwen/Qwen3.5-9B \
  --out        data/local-validation/v2/sub_best.csv
```
(188개라 1~2분. `sub_best.csv` + `sub_best.raw.jsonl` 생성)

### Step 3. 채점
```bash
python3 data/local-validation/v2/score_valset.py \
  data/local-validation/v2/sub_best.csv \
  data/local-validation/v2/answer_key.csv
```
→ 출력되는 표 전체를 사람에게 회신.
...
python3 .../run_valset.py --valset-dir ... --image-dir ... \
  --model-name <후보모델ID> --out data/local-validation/v2/sub_<후보>.csv
python3 .../score_valset.py data/local-validation/v2/sub_<후보>.csv data/local-validation/v2/answer_key.csv
```
