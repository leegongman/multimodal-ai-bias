...
async def call(client,system,content_text,img_b,sfx,sem,temperature=0.0,top_k=1):
    async with sem:
...
async def process(idx,row,image_dir,client,sem,r_sys,v_sys,verify_all,sc_n=1,sc_temp=0.7):
    ...
    # Pass1 (self-consistency: greedy first, then sc_n-1 sampled, majority vote)
    if sc_n<=1:
        out1=await call(client,r_sys,user_text(row,opts),b,img.suffix,sem)
        l1,m1=parse_ans(out1,opts)
        sc_votes=[l1]
    else:
        outs=[await call(client,r_sys,user_text(row,opts),b,img.suffix,sem,temperature=0.0,top_k=1)]
        import asyncio as _a
        sampled=await _a.gather(*[call(client,r_sys,user_text(row,opts),b,img.suffix,sem,temperature=sc_temp,top_k=0) for _ in range(sc_n-1)])
        outs+=list(sampled)
        votes=[parse_ans(o,opts)[0] for o in outs]
        from collections import Counter as _C
        cnt=_C(votes); top=cnt.most_common()
        # tie-break: prefer the greedy (first) answer
        best=top[0][1]; winners=[v for v,c in top if c==best]
        l1 = votes[0] if votes[0] in winners else top[0][0]
        out1=outs[0]; m1="sc_majority"
        sc_votes=votes
    ...
    return idx,{"sample_id":row["sample_id"],"label":final,"stage":stage,
                "reasoner_label":l1,"reasoner_raw":out1,"verifier_raw":out2,"sc_votes":sc_votes}
...
async def main():
    ...
    ap.add_argument("--sc-n",type=int,default=1,help="self-consistency: reasoner 샘플 수(다수결). 1=기존 greedy")
    ap.add_argument("--sc-temp",type=float,default=0.7,help="sc 샘플링 temperature")
    ...
    res=[None]*len(rows)
    tasks=[process(i,r,image_dir,client,sem,r_sys,v_sys,a.verify_all,a.sc_n,a.sc_temp) for i,r in enumerate(rows)]
