...
async def process(idx,row,image_dir,client,sem,r_sys,v_sys,verify_all,no_verify=False):
    ...
    if (not no_verify) and (verify_all or needs_verify(row,opts,l1)):
...
async def main():
    ...
    ap.add_argument("--no-verify",action="store_true",help="reasoner 단독(1pass). verifier 완전 비활성.")
    ...
    res=[None]*len(rows)
    tasks=[process(i,r,image_dir,client,sem,r_sys,v_sys,a.verify_all,a.no_verify) for i,r in enumerate(rows)]
